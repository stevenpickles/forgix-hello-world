...
void     BSP_WatchdogStart(uint32_t timeout_ms);   // watchdog_enable(timeout_ms, true); use 5000 ms
void     BSP_WatchdogFeed(void);
bsp_boot_reason_t BSP_WatchdogBootReason(void);   // POWMAN chip-reset reason + watchdog_enable_caused_reboot()
void     BSP_WatchdogMarkerSet(uint32_t marker);  // watchdog_hw->scratch[0]
uint32_t BSP_WatchdogMarkerGet(void);
void     BSP_WatchdogSnapshotSet(uint32_t slot, uint32_t value);  // scratch[1+slot]
uint32_t BSP_WatchdogSnapshotGet(uint32_t slot);
```
...
void BSP_UsbService(void);   // tud_task() iff FORGIX_FOREGROUND_USB_SERVICE=1, else no-op
bool BSP_UsbPresent(void);   // ADDED: true in the real lib, false in the stub
```

Stub returns `connected=false`, zero counters, no-op service; links into the USB-free target.

`BSP_UsbPresent()` was **added** beyond the original API. The diagnostics layer
...
- `bool BSP_FpgaCdone(void);` (read `PIN_CDONE`) — config-loss indicator.
- `bool BSP_FpgaReconfigure(void);` — recovery entry point. **Implemented as a call to `BSP_FpgaInit()`** returning `result.ready`, rather than as a separate refactor of the static `configure()`: `BSP_FpgaInit` already is exactly "configure, settle, ping-validate", and it additionally refreshes the `fpga_ready` flag that the command layer gates on. Splitting it would have duplicated that. Worst-case duration is roughly 2.5 s (bitstream write + 500 ms CDONE timeout + 1500 ms settle), which fits inside the 5 s watchdog with margin.

`BSP_TimeSleepMs()` was **added** to `bsp_time`, for the boot blink code only.
...
- `application_diagnostics_start()`: report boot reason + retained scratch **before** enabling the watchdog, then `BSP_WatchdogStart(5000)`. The report is emitted as **one uniform line** rather than a different sentence per reason, so the soak harness can match it with a single pattern and the fields line up across runs:
...
- `application_diagnostics_poll()` (first call in runner loop): feed watchdog → `marker_set(LOOP)`; then, in this order, the one-second sample, the 2 Hz LED toggle, **a single LED write**, and the FPGA readback. Ordering the sample first means the heartbeat color always reflects health just read, and it makes the one LED write the same write the FPGA check reads back:
  - `marker_set(USB_SNAPSHOT)`: snapshot `BSP_UsbHealth()`; a stall is measured from the last time `activity_count` or `frame_number` actually changed.
  - `marker_set(FPGA_CHECK)`: `BSP_FpgaCdone()` + `BSP_FpgaPing()` + readback of the LED registers just written. On failure: increment the fpga-fail counter, attempt `BSP_FpgaReconfigure()`, and on success set the recovery signature and rewrite the LED, since a fresh configuration comes up with its registers cleared. **A frozen LED that self-recovers this way proves the MCU was alive and the FPGA lost config — mode 2 attributed without any instruments.**
...
- `firmware/src/application/application_console.c`: set `CONSOLE_READ` before `BSP_ConsoleGetCharTimeoutUs`, `CONSOLE_WRITE` before status/echo/prompt bursts, `COMMAND` before dispatch; **gate the unsolicited status block on `BSP_UsbConnected()`** (approved DTR-gate mitigation).
...
`BSP_TimeNowMs()` with a wrap-safe signed comparison (`(int32_t)(now - deadline) >= 0`);
...
- `option(FORGIX_FOREGROUND_USB_SERVICE OFF)`: when ON, define `PICO_STDIO_USB_ENABLE_IRQ_BACKGROUND_TASK=0` on the app target and `FORGIX_FOREGROUND_USB_SERVICE=1` on `forgix_bsp_usb`; runner then services TinyUSB via `BSP_UsbService()`. Never call `tud_task()` from the foreground while the background IRQ task is active.
...
- Firmware: both images build; `forgix_hello_world.bin` is 209 584 bytes against the 2 MB CI gate. `-DFORGIX_FOREGROUND_USB_SERVICE=ON` was verified to build and to compile `BSP_UsbService` into a `tud_task_ext` tail call with `PICO_STDIO_USB_ENABLE_IRQ_BACKGROUND_TASK=0` applied.
...
| 2026-08-01 | Code finding — stash audit (`stash@{0}^3`) | Restored `application_diagnostics.c` and `led_only_main.c` use `BSP_TimeNowMs()` with wrap-safe signed comparison; no `time_us_32` or other 32-bit µs arithmetic anywhere in the stashed code | The 71.6 min wrap hypothesis has **no candidate site in project code**. It survives only as an SDK-internal possibility, testable solely by logging precise wall-clock time-to-freeze across Run 0 repetitions |
...
| 2026-08-02 | **Schematic finding: populated QSPI PSRAM** | The schematic carries an `APS1604M-SQR` QSPI PSRAM marked do-not-populate, but the IC **is fitted** on the physical board. It shares SCLK and SD0-SD3 with the stacked flash die; only chip select separates them. Firmware references neither PSRAM nor QMI CS1 anywhere, so the part is never initialised and, critically, never deselected. Per **RP2350-E14** the bootrom clears pad isolation only for GPIO 0, not the real CS pin; per **RP2350-E9** an undriven input leaks up to 120 uA and latches near 2 V | **Leading root cause.** An undriven CS drifting into the asserted region lets the PSRAM drive the shared data lines during a flash fetch. Bus contention corrupts the XIP read and hangs the core mid-fetch, which is why no marker was ever recorded. The watchdog resets but the bootrom cannot read flash either, so nothing returns; only a power cycle clears the PSRAM. Explains all-zero flash reads (erased flash reads 0xFF, so zeros mean a second driver), failure with and without USB, failure on every power source, and the drifting time-to-failure. Test: hold PSRAM CS high from the first instruction and re-soak |
| 2026-08-02 | USB-image time-to-failure, three runs | 240 s, 520 s, **554 s**. The last two are 8.7 and 9.2 min, reproducing the originally reported 9-10 min window. Telemetry fully healthy 15-35 s before each death, with `fpga_fail=0` and `fpga_reconfig=0` in every run | The failure is abrupt with no precursor in anything the firmware can measure about itself, consistent with a bus-level fault outside the instrumented paths rather than a software stall. The FPGA hypothesis is effectively eliminated: its health check has never once fired |
| 2026-08-02 | **ROOT CAUSE CONFIRMED - QSPI CS1 deselect, 2 h soak** | Holding GPIO 0 high from board bring-up, the USB image ran the **full 120 minutes** with zero failures: 2182 lines, all 239 pings answered, largest gap 10.1 s, uptime monotonic 61 s to 7208 s with no reset. Baselines on the same image and host were 240 s, 520 s and 554 s, so this is **13x the longest failing run** | **Both failure modes are explained by one board fault.** An `APS1604M-SQR` QSPI device is fitted on a footprint the schematic marks do-not-populate. Its CE# is GPIO 0 with no pull-up, and `FLASH_DEVINFO` reports `CS1_SIZE = 0`, so neither bootrom nor firmware ever drove it. The floating active-low enable let it contend with the boot flash on the shared SD0..SD3, corrupting XIP fetches and hanging the core mid-instruction - hence no progress marker was ever captured, and the watchdog reset into a bootrom that could not read flash either. The FPGA was never involved; its health check never fired in any run. Remaining work: confirm the fitted part's voltage variant, and depopulate it as the schematic intends - the deselect is a firmware mitigation, and cannot cover the window before the first instruction executes |
| 2026-08-02 | **DRAM enabled, second 2 h soak** | With the PSRAM live on the shared bus the USB image again ran the full 120 minutes: 3349 lines, all 239 pings answered, largest gap 10.1 s, uptime monotonic 114 s to 7263 s with zero resets. Every one of the 239 samples reported `psram=2048KiB ok=1`; CDC activity rose monotonically from 31 to 5904; `fpga_fail` and `fpga_reconfig` stayed at 0 | **The 2 MByte of DRAM is usable and does not reintroduce the fault.** This mattered because the first passing soak had the device deselected, so it proved only that keeping it off the bus was safe. Putting it back on the same SCLK and SD0..SD3 the lockup lived on, and holding clean for the same duration, is what makes the memory genuinely available rather than merely detected. `FORGIX_QSPI_PSRAM` now defaults on |
| 2026-08-02 | **The identification probe was the fault, not the device** | The SDK's detection, running in `runtime_init` and sending no reset of its own, sized the part correctly. This project's probe, running later, reported `66 0B 43 57`. The difference was a quad-width reset issued unconditionally just before identification: to a device already in SPI mode those opcodes are not a reset but eight clocks of nonsense across four lines | **An instrument that disturbs its subject produces evidence about itself.** Hours went into explaining a reading that the probe had manufactured -- voltage, clock, QPI mode -- each ruled out by experiment while the real cause sat in the measuring code. The quad reset is now held back until the serial identification actually fails, which is the only case it was built for, and whether it fired is reported. The clock sweep is gone: it had answered its question and was otherwise four extra reset-and-identify sequences per boot on a shared bus |
| 2026-08-02 | **Identity captured cleanly: KGD 0x0B, EID 0x43** | The device can be positively identified, but only in the window before the SDK switches it to QPI for XIP. `psram_eid_to_size` is declared weak precisely so applications can support parts the SDK does not know, so overriding it intercepts the bytes the SDK's own detection has just read -- no extra bus transaction, no disturbance. It reports **KGD 0x0B, EID 0x43** | **This corroborates the discarded probe rather than contradicting it.** That probe returned `00 00 00 00 66 0B 43 57`, whose bytes 5 and 6 are `0B` and `43` -- exactly what the SDK reads from those same offsets. Two independent paths agree, so the response was never garbage: the device genuinely reports this identity, and it is not AP Memory's 0x5D. What was wrong was everything built on top of that reading. The part is therefore not an APS1604M-3SQR by its own account, yet works as 2 MByte of PSRAM and held clean through a two-hour soak. Reading the package marking is the outstanding check, and it is now the only one left |
| 2026-08-02 | Instrumentation defect found during the passing soak | Operator reported the heartbeat alternating ~10 green then ~10 red. The device transmits every 10 s (`APPLICATION_IDLE_STATUS_PERIOD_MS`) while the red state keyed on a 5 s gap in CDC activity, so it tripped on the firmware's own reporting cadence every cycle | A traffic gap alone is not evidence of anything: the cadence is set by console policy and `watch` allows up to 3600 s, so no fixed threshold is safe. Red now requires the transmit FIFO to be full **and** not draining, which is what an endpoint wedge actually looks like, with the activity threshold raised to 30 s and the frame-stall threshold kept at 5 s. Had a real failure occurred, the old red would have been read as a confirmed wedge while meaning nothing |
| 2026-08-02 | Part identification, RP2350A vs RP2354A | picotool cannot name the variant: it reports `type: RP2350` for the whole family and `package: QFN60`, which covers both A parts. Determined indirectly instead. Reads at offsets 0, 2 MiB and 4 MiB return byte-identical vector tables, so the CS0 address wraps and the die is exactly **2 MiB** - matching the W25Q16JV that RP2354 stacks in package. OTP `FLASH_DEVINFO` reports CS0_SIZE = 0xC (16 MiB), the maximum enum, which is a permissive unprogrammed default rather than a real size; our build already overrides it with `PICO_FLASH_SIZE_BYTES = 2 MiB` | **Almost certainly RP2354A.** The 2 MiB CS0 device is not on the PCB - the only QSPI IC fitted is the CS1 part on GPIO 0 - so it must be in package. The schematic showing an RP2350A symbol inside an RP2354A block is consistent with a generic symbol for the shared die and pinout. This holds only if no other flash IC exists on the board, which is worth confirming visually. Note the OTP size field is wrong on this part; anything relying on FLASH_DEVINFO rather than PICO_FLASH_SIZE_BYTES would read 8x too much flash |
| 2026-08-02 | **Design-guide review corrects the diagnosis** | Raspberry Pi's *Hardware design with RP2350* (RP-008280-DS-1) section 3.2 documents a secondary QSPI memory on the same bus, with **GPIO 0 as XIP_CS1n** - exactly this board's layout, so the design is standard and supported. The guide also states the requirement we are missing: fitting the device needs `R13 (10K)` as a pull-up, because *"the default state of GPIO0 is to be pulled low at power-up, which would cause our flash device to fail"*, and the reference schematic repeats it - *"Pull-up R13 will be required as GPIOs default to pull-downs at power-up"*. This board has no such resistor | **Corrects the earlier claim that GPIO 0 was floating.** RP2350 pads default to a pull-down, and the chip select is active low, so the secondary device was *selected* from power-up rather than drifting into selection. It answered traffic meant for the boot flash and drove the shared data lines. Two faults compound: a **board error** (missing mandatory pull-up) and a **firmware error** (never selecting GPIO_FUNC_XIP_CS1, never declaring the device via flash_devinfo, so the pad kept its pull-down default). Firmware can only mitigate: nothing runs before the bootrom's own flash reads. The hardware fix is a 10K pull-up from GPIO 0 to 3V3 |
| 2026-08-02 | ~~APS1604M-SQR datasheet, rev 3.0~~ **WITHDRAWN** | The fitted part is the **1.8 V variant**: `VDD = 1.62 to 1.98 V`, signal table `VDD Power Core supply 1.8V`. It sits on the QSPI bus shared with the stacked flash, which is a 3.3 V W25Q16JVWI, so the bus is 3.3 V. Density is **16 Mb = 2 MByte**, `A[20:0]`, not 16 MByte. Clock rate is 144 MHz wrapped burst and 84 MHz linear-512, against the 75 MHz this bus runs at (clkdiv 2 of 150 MHz), so **clock frequency is comfortably in spec and is not the limiter**. `tCEM` is 3 us at 105 C. Read-ID carries a protocol constraint: it *"can be used ONLY as Power-up initialization after the device Reset tRST 50ns right after Global Reset command"* | **The DRAM is unusable as wired, and the reason is voltage, not configuration.** A 1.8 V part is being driven by a 3.3 V bus, which is why it answers with levels that decode to `66 0B 43 57` instead of AP Memory's `0D 5D` -- the probe is proven good by the CS0 control returning `EF 40 15`. Continued operation stresses it beyond its ratings. It also explains the severity of the original fault: a pull-down held CE# asserted permanently, violating the 3 us tCEM by many orders of magnitude, so the part could never self-refresh while also contending on the shared data lines. `FORGIX_QSPI_PSRAM` now defaults **OFF** so the pin is held firmly deasserted. Board respin needs a 3.3 V part -- the L variant rather than M -- **and** the chip-select pull-up | **This row is wrong: the datasheet fetched was for the plain APS1604M-SQR, the 1.8 V variant. The fitted part is APS1604M-3SQR-SN, where the `-3` denotes 2.7-3.6 V. Voltage is correct for this bus and is not the fault.** |
| 2026-08-02 | **Correction: the part is 3.0 V and correctly specified** | APS1604M-3SQR-SN is the 2.7-3.6 V variant, 16 Mb (2 MByte), rated 133 MHz wrapped burst and 84 MHz linear. Direct mode already clocks the probe at 25 MHz (`DIRECT_CSR.CLKDIV` resets to 6 against a 150 MHz system clock), far inside spec, so **clock frequency is ruled out** and so is voltage. The board's use of GPIO 0 as `XIP_CS1n` matches Raspberry Pi's reference design | The earlier voltage conclusion was drawn from the wrong datasheet and is withdrawn. What still stands: the probe is proven by the CS0 control returning `EF 40 15`; CS1 answers `00 00 00 00 66 0B 43 57` consistently, across reboots, across a global reset, and across all four direct-mode RX delays. That last sweep is **inconclusive rather than negative** -- `flash_do_cmd_cs` calls `connect_internal_flash`, which resets QMI state and discards a `DIRECT_CSR` written beforehand, so the setting never reached the transfer. Testing sampling properly means reimplementing the direct-mode sequence rather than borrowing the SDK's. Still unexplained; the DRAM stays deselected and unused, which costs nothing since board stability is already validated |
