...
| 2026-08-02 | USB shell **with the DRAM live on chip select 1**, `2940de4` | Windows 11 PC, direct port | Soak harness, DTR on, `diag` ping every 30 s | **120 min (full run)** | Healthy | Present throughout | Not needed | **PASS** - 3349 lines, all 239 pings answered, largest gap 10.1 s, uptime monotonic 114 s to 7263 s with **zero resets**. All 239 samples reported `psram=2048KiB ok=1`, CDC activity rose monotonically 31 to 5904, and `fpga_fail`/`fpga_reconfig` stayed 0 throughout |
