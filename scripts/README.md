...
| [check_firmware_layers.py](check_firmware_layers.py) | Protect the dependency boundary between application code and board support, including `_internal.h` ownership: only a module's own implementation files may include its internal header |
...
| Script | What it does |
| --- | --- |
| [test_check_firmware_layers.py](test_check_firmware_layers.py) | Unit tests for the layering checker's internal-header ownership rule, including a whole-repository consistency run |
